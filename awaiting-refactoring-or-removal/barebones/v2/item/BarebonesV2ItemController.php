...
class BarebonesV2ItemController
{
    ...
    {
        if (strpos($this->request_uri, "/api/v2/error") === 0) {
            return true;
        }
        if ($this->getIsOAuth2Request()) {
            return false;
        }
        if (strpos($this->request_uri, "/api/v2/item/") !== 0) {
            return false;
        }
    ...
    protected function requestedItemRoute()
    {
        ...
        $idOrRoute = str_replace("/api/v2/item/", "", rtrim($path, "/"));
        return $idOrRoute;
    }
