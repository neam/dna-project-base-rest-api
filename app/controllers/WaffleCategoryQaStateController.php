...
class WaffleCategoryQaStateController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            array(
                ...
                'roles' => array('WaffleCategoryQaState.*'),
            ),
            array(
                'deny',
                'users' => array('*'),
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = WaffleCategoryQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new WaffleCategoryQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'waffle-category-qa-state-form');

        if (isset($_POST['WaffleCategoryQaState'])) {
            $model->attributes = $_POST['WaffleCategoryQaState'];
        ...
        } elseif (isset($_GET['WaffleCategoryQaState'])) {
            $model->attributes = $_GET['WaffleCategoryQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('WaffleCategoryQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('WaffleCategoryQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new WaffleCategoryQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['WaffleCategoryQaState'])) {
            $model->attributes = $_GET['WaffleCategoryQaState'];
        }
    ...
    {
        $model = WaffleCategoryQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'waffle-category-qa-state-form') {
