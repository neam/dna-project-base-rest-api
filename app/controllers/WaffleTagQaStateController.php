...
class WaffleTagQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = WaffleTagQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new WaffleTagQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'waffle-tag-qa-state-form');

        if (isset($_POST['WaffleTagQaState'])) {
            $model->attributes = $_POST['WaffleTagQaState'];
        ...
        } elseif (isset($_GET['WaffleTagQaState'])) {
            $model->attributes = $_GET['WaffleTagQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('WaffleTagQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('WaffleTagQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new WaffleTagQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['WaffleTagQaState'])) {
            $model->attributes = $_GET['WaffleTagQaState'];
        }
    ...
    {
        $model = WaffleTagQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'waffle-tag-qa-state-form') {
