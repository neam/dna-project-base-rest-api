...
class I18nCatalogQaStateController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            array(
                ...
                'roles' => array('I18nCatalogQaState.*'),
            ),
            array(
                'deny',
                'users' => array('*'),
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = I18nCatalogQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new I18nCatalogQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'i18n-catalog-qa-state-form');

        if (isset($_POST['I18nCatalogQaState'])) {
            $model->attributes = $_POST['I18nCatalogQaState'];
        ...
        } elseif (isset($_GET['I18nCatalogQaState'])) {
            $model->attributes = $_GET['I18nCatalogQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('I18nCatalogQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('I18nCatalogQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new I18nCatalogQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['I18nCatalogQaState'])) {
            $model->attributes = $_GET['I18nCatalogQaState'];
        }
    ...
    {
        $model = I18nCatalogQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'i18n-catalog-qa-state-form') {
