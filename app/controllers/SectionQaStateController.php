...
class SectionQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = SectionQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new SectionQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'section-qa-state-form');

        if (isset($_POST['SectionQaState'])) {
            $model->attributes = $_POST['SectionQaState'];
        ...
        } elseif (isset($_GET['SectionQaState'])) {
            $model->attributes = $_GET['SectionQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('SectionQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('SectionQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new SectionQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['SectionQaState'])) {
            $model->attributes = $_GET['SectionQaState'];
        }
    ...
    {
        $model = SectionQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'section-qa-state-form') {
