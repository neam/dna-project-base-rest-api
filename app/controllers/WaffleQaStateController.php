...
class WaffleQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = WaffleQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new WaffleQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'waffle-qa-state-form');

        if (isset($_POST['WaffleQaState'])) {
            $model->attributes = $_POST['WaffleQaState'];
        ...
        } elseif (isset($_GET['WaffleQaState'])) {
            $model->attributes = $_GET['WaffleQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('WaffleQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('WaffleQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new WaffleQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['WaffleQaState'])) {
            $model->attributes = $_GET['WaffleQaState'];
        }
    ...
    {
        $model = WaffleQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'waffle-qa-state-form') {
