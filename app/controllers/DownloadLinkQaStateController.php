...
class DownloadLinkQaStateController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            array(
                ...
                'roles' => array('DownloadLinkQaState.*'),
            ),
            array(
                'deny',
                'users' => array('*'),
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = DownloadLinkQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new DownloadLinkQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'download-link-qa-state-form');

        if (isset($_POST['DownloadLinkQaState'])) {
            $model->attributes = $_POST['DownloadLinkQaState'];
        ...
        } elseif (isset($_GET['DownloadLinkQaState'])) {
            $model->attributes = $_GET['DownloadLinkQaState'];
        }
    ...
    public function actionEditableSaver()
    {
        ...
        $es = new TbEditableSaver('DownloadLinkQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('DownloadLinkQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new DownloadLinkQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['DownloadLinkQaState'])) {
            $model->attributes = $_GET['DownloadLinkQaState'];
        }
    ...
    {
        $model = DownloadLinkQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'download-link-qa-state-form') {
