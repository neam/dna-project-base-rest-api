...
class HtmlChunkQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = HtmlChunkQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new HtmlChunkQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'html-chunk-qa-state-form');

        if (isset($_POST['HtmlChunkQaState'])) {
            $model->attributes = $_POST['HtmlChunkQaState'];
        ...
        } elseif (isset($_GET['HtmlChunkQaState'])) {
            $model->attributes = $_GET['HtmlChunkQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('HtmlChunkQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('HtmlChunkQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new HtmlChunkQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['HtmlChunkQaState'])) {
            $model->attributes = $_GET['HtmlChunkQaState'];
        }
    ...
    {
        $model = HtmlChunkQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'html-chunk-qa-state-form') {
