...
class VectorGraphicQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = VectorGraphicQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new VectorGraphicQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'vector-graphic-qa-state-form');

        if (isset($_POST['VectorGraphicQaState'])) {
            $model->attributes = $_POST['VectorGraphicQaState'];
        ...
        } elseif (isset($_GET['VectorGraphicQaState'])) {
            $model->attributes = $_GET['VectorGraphicQaState'];
        }
    ...
    public function actionEditableSaver()
    {
        ...
        $es = new TbEditableSaver('VectorGraphicQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('VectorGraphicQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new VectorGraphicQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['VectorGraphicQaState'])) {
            $model->attributes = $_GET['VectorGraphicQaState'];
        }
    ...
    {
        $model = VectorGraphicQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'vector-graphic-qa-state-form') {
