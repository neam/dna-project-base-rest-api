...
class GuiSectionQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = GuiSectionQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new GuiSectionQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'gui-section-qa-state-form');

        if (isset($_POST['GuiSectionQaState'])) {
            $model->attributes = $_POST['GuiSectionQaState'];
        ...
        } elseif (isset($_GET['GuiSectionQaState'])) {
            $model->attributes = $_GET['GuiSectionQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('GuiSectionQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('GuiSectionQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new GuiSectionQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['GuiSectionQaState'])) {
            $model->attributes = $_GET['GuiSectionQaState'];
        }
    ...
    {
        $model = GuiSectionQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'gui-section-qa-state-form') {
