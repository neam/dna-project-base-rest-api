...
class ExamQuestionAlternativeController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            array(
                ...
                'roles' => array('92f9838d.ExamQuestionAlternative.*'),
            ),
            array(
                'deny',
                'users' => array('*'),
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = ExamQuestionAlternative::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new ExamQuestionAlternative;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'exam-question-alternative-form');

        if (isset($_POST['ExamQuestionAlternative'])) {
            $model->attributes = $_POST['ExamQuestionAlternative'];
        ...
        } elseif (isset($_GET['ExamQuestionAlternative'])) {
            $model->attributes = $_GET['ExamQuestionAlternative'];
        }
    ...
    {
        $dataProvider = new CActiveDataProvider('ExamQuestionAlternative');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new ExamQuestionAlternative('search');
        $model->unsetAttributes();

        if (isset($_GET['ExamQuestionAlternative'])) {
            $model->attributes = $_GET['ExamQuestionAlternative'];
        }
    ...
    {
        $model = ExamQuestionAlternative::model()->findByPk($id);
        if ($model === null) {
