...
class SlideshowFileQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = SlideshowFileQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new SlideshowFileQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'slideshow-file-qa-state-form');

        if (isset($_POST['SlideshowFileQaState'])) {
            $model->attributes = $_POST['SlideshowFileQaState'];
        ...
        } elseif (isset($_GET['SlideshowFileQaState'])) {
            $model->attributes = $_GET['SlideshowFileQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('SlideshowFileQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('SlideshowFileQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new SlideshowFileQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['SlideshowFileQaState'])) {
            $model->attributes = $_GET['SlideshowFileQaState'];
        }
    ...
    {
        $model = SlideshowFileQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'slideshow-file-qa-state-form') {
