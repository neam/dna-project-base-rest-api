...
class SnapshotQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = SnapshotQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new SnapshotQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'snapshot-qa-state-form');

        if (isset($_POST['SnapshotQaState'])) {
            $model->attributes = $_POST['SnapshotQaState'];
        ...
        } elseif (isset($_GET['SnapshotQaState'])) {
            $model->attributes = $_GET['SnapshotQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('SnapshotQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('SnapshotQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new SnapshotQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['SnapshotQaState'])) {
            $model->attributes = $_GET['SnapshotQaState'];
        }
    ...
    {
        $model = SnapshotQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'snapshot-qa-state-form') {
