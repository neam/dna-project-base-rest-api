...
class PoFileQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = PoFileQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new PoFileQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'po-file-qa-state-form');

        if (isset($_POST['PoFileQaState'])) {
            $model->attributes = $_POST['PoFileQaState'];
        ...
        } elseif (isset($_GET['PoFileQaState'])) {
            $model->attributes = $_GET['PoFileQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('PoFileQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('PoFileQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new PoFileQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['PoFileQaState'])) {
            $model->attributes = $_GET['PoFileQaState'];
        }
    ...
    {
        $model = PoFileQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'po-file-qa-state-form') {
