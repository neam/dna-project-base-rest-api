...
class VideoFileQaStateController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if (!isset($_GET['id']) && isset($_GET['id'])) {
            $model = VideoFileQaState::model()->find(
                'id = :id',
                array(
                    ':id' => $_GET['id']
                )
    ...
    {
        $model = new VideoFileQaState;
        $model->scenario = $this->scenario;

        $this->performAjaxValidation($model, 'video-file-qa-state-form');

        if (isset($_POST['VideoFileQaState'])) {
            $model->attributes = $_POST['VideoFileQaState'];
        ...
        } elseif (isset($_GET['VideoFileQaState'])) {
            $model->attributes = $_GET['VideoFileQaState'];
        }
    ...
    {
        $es = new TbEditableSaver('VideoFileQaState'); // classname of model to be updated
        $es->update();
    }
    ...
    {
        $dataProvider = new CActiveDataProvider('VideoFileQaState');
        $this->render('index', array('dataProvider' => $dataProvider,));
    }

    public function actionAdmin()
    {
        $model = new VideoFileQaState('search');
        $model->unsetAttributes();

        if (isset($_GET['VideoFileQaState'])) {
            $model->attributes = $_GET['VideoFileQaState'];
        }
    ...
    {
        $model = VideoFileQaState::model()->findByPk($id);
        if ($model === null) {
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'video-file-qa-state-form') {
