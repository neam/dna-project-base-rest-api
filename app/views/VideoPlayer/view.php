...
<?php if ($this->videoExists()): ?>
    <video width="604" height="340" controls="controls" style="<?php echo $this->getStyles(); ?>">
        <!-- Not used preload="none" -->
        <?php foreach ($p3MediaFiles as $p3Media): ?>
            <source type="<?php echo $p3Media->mime_type; ?>" src="<?php echo $this->getVideoUrl($p3Media->id); ?>">
        <?php endforeach; ?>
        <track src="<?php echo $this->getSubtitleUrl(); ?>" kind="subtitles" srclang="<?php echo e($srcLang); ?>"
               default></track>
