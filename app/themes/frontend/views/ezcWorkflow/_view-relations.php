...
<ul>
    ...
    <?php
    $records = $model->ezcExecutions(array('limit' => 250, 'scopes' => ''));
    if (is_array($records)) {
        foreach ($records as $i => $relatedModel) {
            echo '<li>';
            echo CHtml::link(
                '<i class="icon icon-arrow-right"></i> ' . $relatedModel->itemLabel,
                array('//ezcExecution/view', 'execution_id' => $relatedModel->execution_id)
            );
            echo CHtml::link(
                ' <i class="icon icon-pencil"></i>',
                array('//ezcExecution/update', 'execution_id' => $relatedModel->execution_id)
            );
            echo '</li>';
        }
    }
